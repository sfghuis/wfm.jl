...
# # Test probability_zero_customers function
# @testset "Probability Zero Customers Tests" begin
#     test_data = ErlangCData(arrivals=200, interval=60, handling_time=3, speed_of_answer=20/60, num_agents=10, service_level=.80)
#     @test isapprox(ErlangC.probability_zero_customers(data), expected_result, atol=1e-5)
# end

# Test probability_queued function
@testset "Probability Queued Tests" begin
    test_data = ErlangCData(arrivals=100, interval=30, handling_time=3, speed_of_answer=20/60, num_agents=10, service_level=.80)
    @test isapprox(ErlangC.probability_queued(test_data), 0.174, atol=1e-5)
end

# # Test average_customers function
# @testset "Average Customers Tests" begin
#     test_data = ErlangCData(arrivals=200, interval=60, handling_time=3, speed_of_answer=20/60, num_agents=10, service_level=.80)
#     @test isapprox(ErlangC.average_customers(data), expected_result, atol=1e-5)
# end

# # Test agents_required function
# @testset "Agents Required Tests" begin
#     test_data = ErlangCData(arrivals=200, interval=60, handling_time=3, speed_of_answer=20/60, num_agents=10, service_level=.80)
#     @test isapprox(ErlangC.agents_required(data), expected_result, atol=1e-5)
# end
